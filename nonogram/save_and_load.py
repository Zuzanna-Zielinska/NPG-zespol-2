...
def save_as_file(new_pictures_matrix: List[List[int]], new_file_name: str, old_file_name: str = 'None',
                 new_pictures_name: str = 'None'):
...
# -------Pomocnicza fukcja do kopiowania plików-------
def change_to_solved(change: bool,id: int, file_name: str):
    with open(file_name, 'rb') as input:  # Wczytywanie danych
        pictures = pickle.load(input)

    for p in pictures:  # Szukanie włściwego id i oznaczenie jako uzupełnionego
        if p.id == id:
            p.is_solved = change  # Oznaczenie jako wypełnionego
